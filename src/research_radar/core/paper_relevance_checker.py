...
class PaperRelevanceChecker:
    ...
    def __init__(self, metadata: Dict, required_keywords: List[str], min_match_threshold: int = 5):
        """
    ...
    def check_relevance(self) -> bool:
        ...
        # Keywords are stored as a single string separated by " | " in the metadata
        ai_keywords_string: str = self.metadata.get("ai_keywords", "")
        
        ai_keywords_set = {
            k.strip().lower() 
            for k in ai_keywords_string.split(" | ") if k.strip()
        }
        
        required_keywords_set = {
            k.strip().lower() 
            for k in self.required_keywords if k.strip()
        }
        ...
        logger.info(
            f"Paper {self.paper_id} Relevance Check Results:"
        )
